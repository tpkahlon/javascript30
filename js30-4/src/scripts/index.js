...
const app = data =>
data.sort((lastColor, nextColor) => lastColor.colorId > nextColor.colorId ? 1 : -1)
.map(item => makeAppBoxes(item));

const taskOne = data =>
data.filter(color => color.name.includes('Purple'))
.sort((lastColor, nextColor) => lastColor.colorId > nextColor.colorId ? 1 : -1)
...
data.filter(color => color.name.includes('Blue'))
...
const taskFive = data =>
data.filter(color => color.name.includes('Grey'))
.sort((lastColor, nextColor) => lastColor.name > nextColor.name ? -1 : 1)
.map(item => taskFiveBoxes(item));

const taskSix = data =>
data.filter(color => color.name.includes('de'))
.sort((lastColor, nextColor) => lastColor.name > nextColor.name ? 1 : -1)
.map(item => taskSixBoxes(item));

const taskSeven = data =>
data.sort((lastColor, nextColor) => {
...
const makeAppBoxes = item => {
  const rgbValue = Object.keys(item.rgb).map(k => item.rgb[k]).join(',');
  const hslValue = Object.keys(item.hsl).map(k => item.hsl[k]).join(',');
